...
func main() {
	...
	if err := db.Get(&kuni, "SELECT Code,Population FROM country WHERE Code='"+kunihosii+"'"); errors.Is(err, sql.ErrNoRows) {
		log.Printf("no such city Name = %s", "Tokyo")
	} else if err != nil {
		log.Fatalf("DB Error: %s", err)
	}
	...
	fmt.Printf(city.Name+"は"+kuni.Name+"%d\n", (city.Population/kuni.Population)*100)
	print(kuni.Population)
}
