...
func addHandler(c echo.Context) error {
	...
	var city City
	if err := db.Get(&city, "INSERT INTO city (Name, CountryCode, District, Population) VALUES ("+string(data.Name)+", "+string(data.CountryCode)+", "+string(data.District)+", "+string(data.Population)); errors.Is(err, sql.ErrNoRows) {
