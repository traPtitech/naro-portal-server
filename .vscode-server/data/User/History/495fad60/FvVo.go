...
func main() {
	...
	if err := db.Get(&kuni, "SELECT Code, Name, Language FROM country JOIN countrylanguage ON Code = CountryCode WHERE Language = 'English' LIMIT 10;"); errors.Is(err, sql.ErrNoRows) {
		log.Printf("no such city Name = %s", "Tokyo")
	} else if err != nil {
		log.Fatalf("DB Error: %s", err)
	}
